<template>
  <BaseSearchCriteria ref="baseSearchRef" :searchFields="searchFields" :fieldLabelMap="fieldLabelMap">
    ...
    <SearchTypesDropdown :options="searchTypes" :otherDropdowns="[dropdownGivenname, dropdownFather, dropdownMother, dropdownVillage]" v-model="selectedSearchTypeSurname" ref="dropdownSurname"/>
   
    <input v-model="searchFields.given_name" type="text" class="filter-input" spellcheck="false" placeholder="Given Name">
    <SearchTypesDropdown :options="searchTypes" :otherDropdowns="[dropdownSurname, dropdownFather, dropdownMother, dropdownVillage]" v-model="selectedSearchTypeGivenname" ref="dropdownGivenname"/>

    <label class="option-label">Parent's Names:</label>

    <input v-model="searchFields.father_or_spouse" type="text" class="filter-input" spellcheck="false" placeholder="Father's Given Name">
    <SearchTypesDropdown :options="searchTypes" :otherDropdowns="[dropdownSurname, dropdownGivenname, dropdownMother, dropdownVillage]" v-model="selectedSearchTypeFather" ref="dropdownFather"/>
    
    <input v-model="searchFields.mother_or_spouse" type="text" class="filter-input" spellcheck="false" placeholder="Mother's Surname or Given Name">
    <SearchTypesDropdown :options="searchTypesPlace" :otherDropdowns="[dropdownSurname, dropdownGivenname, dropdownFather, dropdownVillage]" v-model="selectedSearchTypeMother" ref="dropdownMother"/>
    ...
    <SearchTypesDropdown :options="searchTypesPlace" :otherDropdowns="[dropdownSurname, dropdownGivenname, dropdownFather, dropdownMother]" v-model="selectedSearchTypeVillage" ref="dropdownVillage"/>
...
const fieldLabelMap: { [key: string]: string } = {
  ...
  event_year: 'Birth Year',
  father_or_spouse: 'Father\'s Given Name',
  mother_or_spouse: 'Mother\'s Surname or Given Name',
  village: 'Birth Place'
};
...
const selectedSearchTypeMother = ref(searchTypesPlace.value[0]);
