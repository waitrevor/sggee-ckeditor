  ...
  <div class="person-card">
    <div class="name">
      <div class="name-container">
        <strong style="margin-right: 3px;">Groom: </strong>
        <template v-if="individualData.groom_first">
          {{ individualData.groom_first + ' ' }}
        </template>
        <template v-if="individualData.groom_last">
          {{ individualData.groom_last }}
        </template>
      </div>
      <div class="name-container">
        <strong style="margin-right: 3px;">Bride: </strong>
        <template v-if="individualData.bride_first">
          {{ individualData.bride_first + ' ' }}
        </template>
        <template v-if="individualData.bride_last">
          {{ individualData.bride_last }}
        </template>
      </div>
    ...
    <div class="info">
      ...
      <div>
        ...
        <template v-if="individualData.place">
        ...
        <div>
          ...
          <template v-if="individualData.day_month">
            {{ individualData.day_month + ' ' }}
          </template>
          <template v-if="individualData.year">
            {{ individualData.year }}
          </template>
          <template v-if="!individualData.day_month && !individualData.year">
            <span class="missing-text">{{ missingPlaceholdlerText }}</span>
          </template>
        </div>
      </div>
    ...
    <div class="parents">
      <div>
        <strong>Prev Wife / Groom Parents: </strong>
        <template v-if="individualData.prev_wife_or_groom_parent">
          {{ individualData.prev_wife_or_groom_parent }}
        </template>
        <template v-else>
          <span class="missing-text">{{ missingPlaceholdlerText }}</span>
        </template>
      ...
      <div>
        <strong>Prev Husband / Bride Parents: </strong>
        <template v-if="individualData.prev_husb_or_bride_parent">
          {{ individualData.prev_husb_or_bride_parent }}
        </template>
        <template v-else>
          <span class="missing-text">{{ missingPlaceholdlerText }}</span>
        </template>
